...
export default function QuotationForm() {
  ...
  useEffect(() => {
    if (existing) {
      ...
      setValidUntil(existing.validUntil || addDaysIso(15));
  ...
  const mutation = useMutation({
    mutationFn: () => (isEdit ? updateQuotation(companyId!, id!, buildPayload()) : createQuotation(companyId!, buildPayload())),
    onSuccess: (q) => {
      queryClient.invalidateQueries({ queryKey: ['quotations', companyId] });
      notify(isEdit ? 'Quotation updated' : `Quotation ${q.quotationNumber} created`);
      navigate(`/quotations/${q.id}`);
    },
    onError: (err) => setError(apiErrorMessage(err, 'Could not save quotation')),
  });
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 pb-24">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-xl font-semibold text-slate-900">{isEdit ? `Edit ${existing?.quotationNumber}` : 'New Quotation'}</h1>
      </div>
      ...
      <Card>
        ...
        <CardBody className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div />
          <Input label="Quotation date" type="date" required value={quotationDate} onChange={(e) => setQuotationDate(e.target.value)} />
          <Input label="Valid until" type="date" value={validUntil} onChange={(e) => setValidUntil(e.target.value)} hint="Offer expiry" />
        </CardBody>
      </Card>
